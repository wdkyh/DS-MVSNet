...
class MVSDataset(Dataset):
    ...
    def build_list(self):
        ...
        print("dataset {0: <8} metas: {1: <5}".format(self.mode, len(metas)))
        return metas
    ...
    def __getitem__(self, idx):
        ...
        imgs = []
        mask = None # reference view mask
        masks = []  # all views mask
        depth_ms = None
        depth_values = None
        proj_matrices = []
        ...
        for i, vid in enumerate(view_ids):
            ...
            # all views mask
            mask_view = Image.open(mask_filename_hr)
            np_mask_view = np.array(mask_view, dtype=np.float32)
            np_mask_view = (np_mask_view > 10).astype(np.float32)
            np_mask_view = self.prepare_img(np_mask_view)
            masks.append(np_mask_view)
